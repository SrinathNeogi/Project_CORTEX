...
class PDFFinder:
    ...
    # Words to ignore
    STOPWORDS = ["find", "search", "open", "locate", "pdf", "file"]
    ...
    def get_response(self, command: str):
        ...
        # Trigger only if PDF-related
        if "pdf" not in command:
            return None, None

        query = self.extract_query(command)

        if not query:
            return "Please tell me the name of the PDF file.", None
            ...
            for root, dirs, files in os.walk(path):
                for file in files:
                    if file.lower().endswith(".pdf"):
                        ...
                        filename = file.lower().replace(".pdf", "")
                        all_files.append((filename, full_path, file))

        if not all_files:
            return "No PDF files found in search locations.", None
        ...
        return "Sorry, I could not find that PDF file.", None